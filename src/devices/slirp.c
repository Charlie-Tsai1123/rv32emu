...
void net_slirp_cleanup(net_user_options_t *usr)
{
    ...
    if (usr->guest_to_host_channel[SLIRP_READ_SIDE] > 0)
        close(usr->guest_to_host_channel[SLIRP_READ_SIDE]);
    if (usr->guest_to_host_channel[SLIRP_WRITE_SIDE] > 0)
        close(usr->guest_to_host_channel[SLIRP_WRITE_SIDE]);
    if (usr->host_to_guest_channel[SLIRP_READ_SIDE] > 0)
        close(usr->host_to_guest_channel[SLIRP_READ_SIDE]);
    if (usr->host_to_guest_channel[SLIRP_WRITE_SIDE] > 0)
        close(usr->host_to_guest_channel[SLIRP_WRITE_SIDE]);
...
int net_slirp_poll(net_user_options_t *usr)
{
    ...
    if (usr->host_to_guest_channel[SLIRP_READ_SIDE] >= 0)
        slirp_add_poll_socket(usr->host_to_guest_channel[SLIRP_READ_SIDE],
    ...
    if (usr->host_to_guest_channel[SLIRP_READ_SIDE] >= 0) {
        for (int i = 0; i < usr->pfd_len; i++) {
            if (usr->pfd[i].fd == usr->host_to_guest_channel[SLIRP_READ_SIDE] &&
